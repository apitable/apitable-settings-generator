...
import { Transformer } from "../lib/transfomer.class";
...
describe("test APITable settings generator", () => {
  ...
  it("should single row settings transform ok", async () => {
    ...
    const genSettings = tf.parseTable(firstTableConfig, requestedDataMap[firstTableConfig.datasheetId])
  ...
  it("should single column settings transform ok", async () => {
    ...
    const genSettings = tf.parseTable(columnTableConfig, requestedDataMap[columnTableConfig.datasheetId])
    expect(genSettings).not.toBeNull();
    expect(genSettings).not.toBeUndefined();
